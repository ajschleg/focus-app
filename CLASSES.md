...
### Class art briefs (style guide + per-class, all DRAFT)

**The style: Oblivion-manual figures, modernized.** Each class is one
full-body figure in a loose ink-and-wash sketch — warm sepia line, visible
brushwork, no background beyond a ground shadow — like a page from a worn
RPG manual. The modern touch is twofold. *Rendering:* clean linework over
the wash, exactly one flat accent color per figure (the class's tint),
parchment ground in light mode swapping to slate in dark, and a silhouette
that still reads at badge size (~84 pt). *Subject:* every figure strikes a
fantasy pose while holding modern life — that's the system's whole joke
